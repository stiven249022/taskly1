...
<head>
    ...
    <title>Taskly - Editar Materia</title>
                ...
                <div class="flex items-center justify-between px-6 py-4">
                    <div class="flex items-center">
                        ...
                        <div class="flex items-center space-x-2">
                            ...
                            <h2 class="text-xl font-semibold text-gray-800">Editar Materia</h2>
                        </div>
                    </div>
                </div>
            </header>
                ...
                <div class="max-w-4xl mx-auto">
                    ...
                    <div class="bg-white rounded-lg shadow-lg border border-gray-200 overflow-hidden">
                        ...
                        <div class="px-6 py-4 border-b border-gray-200 bg-gray-50">
                            ...
                            <p class="text-sm text-gray-500 mt-1">Modifica los campos que desees cambiar</p>
                        </div>
                        
                        <!-- Form -->
                        <form method="POST" action="{{ route('courses.update', $course) }}" class="px-6 py-6">
                            @csrf
                            @method('PUT')
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div>
                                    ...
                                    <select id="credits" name="credits" required 
                                        ...
                                        @for($i = 1; $i <= 6; $i++)
                                            <option value="{{ $i }}" {{ old('credits', $course->credits) == $i ? 'selected' : '' }}>{{ $i }} crédito{{ $i > 1 ? 's' : '' }}</option>
                                        @endfor
                                    </select>
                                    @error('credits')
                                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                ...
                                <div>
                                    ...
                                    <select id="semester" name="semester" required 
                                        ...
                                        @for($i = 1; $i <= 10; $i++)
                                            <option value="Semestre {{ $i }}" {{ old('semester', $course->semester) == "Semestre $i" ? 'selected' : '' }}>Semestre {{ $i }}</option>
                                        @endfor
                                    </select>
                                    @error('semester')
                                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                ...
                                <div class="md:col-span-2">
                                    ...
                                    <div class="grid grid-cols-8 gap-3">
                                        ...
                                        @foreach($colors as $color)
                                            <div class="color-option w-10 h-10 rounded-full cursor-pointer border-2 border-gray-200 {{ old('color', $course->color) == $color ? 'selected' : '' }}" 
                                                 style="background-color: {{ $color }}"
                                                 data-color="{{ $color }}"
                                                 onclick="selectColor('{{ $color }}')"></div>
                                        @endforeach
                                    </div>
                                    <input type="hidden" name="color" id="selectedColor" value="{{ old('color', $course->color) }}" required>
                                    @error('color')
                                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                    @enderror
                                </div>
                                ...
                                <div class="md:col-span-2">
                                    ...
                                    <textarea id="description" name="description" rows="4" 
                                              class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500" 
                                              placeholder="Describe brevemente el contenido y objetivos de la materia...">{{ old('description', $course->description) }}</textarea>
                                    @error('description')
                                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                    @enderror
                                </div>
                            ...
                            <div class="flex justify-between mt-8 pt-6 border-t border-gray-200">
                                ...
                                <button type="submit" 
                                    ...
                                    <i class="fas fa-check-circle mr-2"></i> Actualizar Materia
                                </button>
                            </div>
                        </form>
                    </div>
